...
path_dataset_dir = "scraping cases\Yearwise_data\ourt" # give the path of directory where the dataset is saved
...
for i in progressbar.progressbar(range(len(files))):
    ...
    fff = open("scraping cases\Yearwise_data\Preprocessed" + files[i] + ".txt" ,"w+") #finally writing the preprocessed file in utput directory
    fff.write(text_new)
    fff.close()